...
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, onAuthStateChanged, sendEmailVerification, fetchSignInMethodsForEmail } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js";
...
function setupAuthListeners() {
    ...
    document.getElementById('showLoginBtn').addEventListener('click', () => {
        ...
        document.getElementById('authTitle').innerText = 'Masuk ke Satriawa';
    });
    ...
    document.getElementById('registerBtn').addEventListener('click', async () => {
        ...
        const tanggalLahir = document.getElementById('registerTanggalLahir').value;

        if (!nama || !email || !password || !confirmPassword || !tanggalLahir) {
        ...
        try {
            ...
            alert('Pendaftaran berhasil! Silakan cek email Anda untuk verifikasi sebelum melanjutkan.');
            ...
            document.getElementById('registerTanggalLahir').value = '';
        } catch (error) {
    ...
    document.getElementById('loginBtn').addEventListener('click', async () => {
        const emailOrNama = document.getElementById('loginEmail').value.trim();
        const password = document.getElementById('loginPassword').value;

        if (!emailOrNama || !password) {
            alert('Email/Nama dan Password harus diisi!');
            return;
        }

        try {
            let email = emailOrNama;
            
            if (!emailOrNama.includes('@')) {
                const usersRef = collection(db, "users");
                const q = query(usersRef, where("name", "==", emailOrNama));
                const querySnapshot = await getDocs(q);
                
                if (querySnapshot.empty) {
                    const pendingRef = collection(db, "pending_users");
                    const qPending = query(pendingRef, where("nama", "==", emailOrNama));
            ...
            if (error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password') {
                pesan += 'Email/Nama atau password salah';
            } else if (error.code === 'auth/invalid-email') {
                pesan += 'Email tidak valid';
            } else {
                pesan += error.message;
...
function setupUIListeners() {
    ...
    document.getElementById('setupSubmitBtn').addEventListener('click', async () => {
        ...
        const pendingDoc = await getDoc(doc(db, "pending_users", auth.currentUser.uid));
        ...
        const userData = { 
            ...
            name: name, 
            email: pendingData.email,
            tanggalLahir: pendingData.tanggalLahir,
            photoUrl: photoUrl, 
            createdAt: serverTimestamp() 
        };
        
        await setDoc(doc(db, "users", auth.currentUser.uid), userData);
        await deleteDoc(doc(db, "pending_users", auth.currentUser.uid));
